...
// kimi
void StreamParams::SetOuterSsrcs(int num_layers,
								 bool generate_fid,
								 bool generate_fec_fr,
								 rtc::UniqueRandomIdGenerator* ssrc_generator,
								 absl::optional<uint32_t> out_ssrc,
								 absl::optional<uint32_t> out_rtx_ssrc) {
	RTC_DCHECK_GE(num_layers, 0);
	RTC_DCHECK(ssrc_generator);
	...
	for (int i = 0; i < num_layers; ++i) {
		...
		if (i == 0) {
			if (out_ssrc.has_value()) {
				...
				ssrc = ssrc_generator->GenerateId();
			}
		}
		else {
			ssrc = ssrc_generator->GenerateId();
	...
	if (generate_fid) {
		int i = 0;
		for (uint32_t ssrc : primary_ssrcs) {
			if (i == 0 && out_rtx_ssrc.has_value()) {
				AddFidSsrc(ssrc, out_rtx_ssrc.value());
			}
			else {
				AddFidSsrc(ssrc, ssrc_generator->GenerateId());
			}
