...
#else 
  RTC_DCHECK_EQ(0, memcmp(packet1 + 12, packet2 + 12, 4));
  RTC_DCHECK_EQ('\xBE', packet1[12]);
  RTC_DCHECK_EQ('\xDE', packet1[13]);
#endif
...
#else
	RTC_DCHECK_LE(extension_data1, extension_end1);
#endif
...
#else
	  RTC_DCHECK_EQ(0, memcmp(extension_data1, extension_data2, len));
#endif
    } else {
#ifndef UNUSED_GTEST
      EXPECT_NE(0, memcmp(extension_data1, extension_data2, len));
#else
	  RTC_DCHECK_NE(0, memcmp(extension_data1, extension_data2, len));
#endif
    }
