...
class SctpTransportMap {
 public:
  ...
  bool PostToTransportThread(uintptr_t id,
	...
	transport->network_thread_->PostTask(SafeTask(
		transport->task_safety_.flag(), [transport, action]() { action(transport); }));
...
int SctpTransport::OnDataOrNotificationFromSctp(const void* data,
  ...
  if (flags & MSG_NOTIFICATION) {
	...
	network_thread_->PostTask(SafeTask(
		task_safety_.flag(), [this, notification = std::move(notification)]() {
  ...
  network_thread_->PostTask(webrtc::SafeTask(
	  task_safety_.flag(), [this, params = std::move(params),
