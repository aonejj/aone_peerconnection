...
static RTCErrorType ParseIceServerUrl(
    const PeerConnectionInterfaceDefs::IceServer& server,		
  ...
  switch (service_type) {
    ...
    case TURNS: {
      ...
      if (server.tls_cert_policy ==
          PeerConnectionInterfaceDefs::kTlsCertPolicyInsecureNoCheck) {		
...
RTCErrorType ParseIceServers(
    const PeerConnectionInterfaceDefs::IceServers& servers,		
    cricket::ServerAddresses* stun_servers,
    std::vector<cricket::RelayServerConfig>* turn_servers) {
  for (const PeerConnectionInterfaceDefs::IceServer& server : servers) {	
