...
#include "../rtc_pc_src/pc/RTCChannelManager.h"
...
static StreamParams CreateStreamParamsForNewSenderWithSsrcs(
  ...
  if (sender._out_set_ssrc.has_value()) {	
	  result.SetOuterSsrcs(sender.num_sim_layers, include_rtx_streams,
		  include_flexfec_stream, ssrc_generator,
		  sender._out_set_ssrc, sender._out_set_rtx_ssrc);
  }
  else {
...
static bool CreateMediaContentAnswer(
  ...
  answer->set_conference_mode(true);
...
MediaSessionDescriptionFactory::MediaSessionDescriptionFactory(
    RTCChannelManager* channel_manager,				
    const TransportDescriptionFactory* transport_desc_factory,
    rtc::UniqueRandomIdGenerator* ssrc_generator)
