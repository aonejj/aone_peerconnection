...
JsepTransportController::JsepTransportController(
  ...
  invoker_.SetRTCThreadMananger(config_._rtc_thread_manager);
}
...
#ifndef UNUSED_PEER_CONNECTION
  if (config_.bundle_policy == PeerConnectionInterface::kBundlePolicyMaxBundle &&
#else
  if (config_.bundle_policy == PeerConnectionInterfaceDefs::kBundlePolicyMaxBundle &&
#endif
...
#else
  if (config_.rtcp_mux_policy == PeerConnectionInterfaceDefs::kRtcpMuxPolicyRequire &&
#endif
...
RTCError JsepTransportController::MaybeCreateJsepTransport(
  ...
  cricket::ConnectionRole connection_role = description.GetTransportDescriptionByName(content_info.name)->connection_role;
  if (!local && 
		  (connection_role == cricket::ConnectionRole::CONNECTIONROLE_ACTIVE || 
		  connection_role == cricket::ConnectionRole::CONNECTIONROLE_ACTPASS)) {
	  rtp_dtls_transport->SetDtlsRole(rtc::SSLRole::SSL_SERVER);
  }
...
#else
  PeerConnectionInterfaceDefs::IceConnectionState new_ice_connection_state =
	  PeerConnectionInterfaceDefs::IceConnectionState::kIceConnectionNew;
  PeerConnectionInterfaceDefs::PeerConnectionState new_combined_state =
	  PeerConnectionInterfaceDefs::PeerConnectionState::kNew;
#endif
...
#else 
	new_ice_connection_state = PeerConnectionInterfaceDefs::kIceConnectionFailed;
#endif 
  } else if (total_ice_disconnected > 0) {
    // None of the previous states apply and any RTCIceTransports are in the
    // "disconnected" state.
#ifndef UNUSED_PEER_CONNECTION	
    new_ice_connection_state = PeerConnectionInterface::kIceConnectionDisconnected;
#else
	new_ice_connection_state = PeerConnectionInterfaceDefs::kIceConnectionDisconnected;
#endif
...
#else
	new_ice_connection_state = PeerConnectionInterfaceDefs::kIceConnectionNew;
#endif
...
#else
	new_ice_connection_state = PeerConnectionInterfaceDefs::kIceConnectionChecking;
#endif
...
#else
	new_ice_connection_state = PeerConnectionInterfaceDefs::kIceConnectionCompleted;
#endif
...
#else
	new_ice_connection_state = PeerConnectionInterfaceDefs::kIceConnectionConnected;
#endif
  } else {
    RTC_NOTREACHED();
  }
...
#ifndef UNUSED_PEER_CONNECTION	
    if (standardized_ice_connection_state_ == PeerConnectionInterface::kIceConnectionChecking &&
        new_ice_connection_state == PeerConnectionInterface::kIceConnectionCompleted) {
#else
	  if (standardized_ice_connection_state_ == PeerConnectionInterfaceDefs::kIceConnectionChecking &&
		  new_ice_connection_state == PeerConnectionInterfaceDefs::kIceConnectionCompleted) {
#endif 
      // Ensure that we never skip over the "connected" state.
      invoker_.AsyncInvoke<void>(RTC_FROM_HERE, signaling_thread_, [this] {
#ifndef UNUSED_PEER_CONNECTION	
        signal_standardized_ice_connection_state_.Send(PeerConnectionInterface::kIceConnectionConnected);
#else
		signal_standardized_ice_connection_state_.Send(PeerConnectionInterfaceDefs::kIceConnectionConnected);
#endif
      });
    }
...
#else
	new_combined_state = PeerConnectionInterfaceDefs::PeerConnectionState::kFailed;
#endif
  } else if (total_ice_disconnected > 0) {
...
#else
	new_combined_state = PeerConnectionInterfaceDefs::PeerConnectionState::kDisconnected;
#endif 
...
#else
	new_combined_state = PeerConnectionInterfaceDefs::PeerConnectionState::kNew;	
#endif
...
#ifndef UNUSED_PEER_CONNECTION	
    new_combined_state = PeerConnectionInterface::PeerConnectionState::kConnecting;
#else
	new_combined_state = PeerConnectionInterfaceDefs::PeerConnectionState::kConnecting;
#endif
...
#ifndef UNUSED_PEER_CONNECTION	
    new_combined_state = PeerConnectionInterface::PeerConnectionState::kConnected;
#else
	new_combined_state = PeerConnectionInterfaceDefs::PeerConnectionState::kConnected;
#endif
  } else {
    RTC_NOTREACHED();
  }
