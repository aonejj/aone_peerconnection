...
#include "../../rtc_pc_src/_deprecate_defines.h"	// add kimi
...
#define RTC_LOG_THREAD_BLOCK_COUNT(x)                                        \
...
public:
	ThreadManager(void* rtc_thread_manager);	
	~ThreadManager();

	void* GetRTCThreadMananger();
...
class RTC_LOCKABLE RTC_EXPORT Thread : public webrtc::TaskQueueBase {
 public:
  ...
  static Thread* Current(void* rtc_thread_manager);
  ...
  class ScopedDisallowBlockingCalls {
   public:
	ScopedDisallowBlockingCalls(void* rtc_thread_manager);
  ...
  class ScopedCountBlockingCalls {
   public:
	   ScopedCountBlockingCalls(std::function<void(uint32_t, uint32_t)> callback, void* rtc_thread_manager);
 ...
 private:
  ...
  void* _rtc_thread_manager;	// add kimi
};
