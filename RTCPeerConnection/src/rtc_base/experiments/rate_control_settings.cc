...
RateControlSettings::RateControlSettings(
	const WebRtcKeyValueConfig* const key_value_config) {
	congestion_window_config_.queue_size_ms = kDefaultAcceptedQueueMs;
	congestion_window_config_.min_bitrate_bps = kDefaultMinPushbackTargetBitrateBps;
}
#endif 
...
#else 
  return RateControlSettings(nullptr);
#endif
}
