...
void RecursiveCriticalSection::Enter() const RTC_EXCLUSIVE_LOCK_FUNCTION() {
  ...
  do {
    ...
    if (!IsThreadRefEqual(owning_thread_, self)) {
	  if (lock_queue_.load(std::memory_order_acquire) == 0) {	
		if (lock_queue_.compare_exchange_strong(0,1) == 0) {	
          have_lock = true;
          break;
        }
      }
    } else {
	  lock_queue_.fetch_add(1, std::memory_order_relaxed);
      have_lock = true;
      break;
    }
  ...
  if (!have_lock && lock_queue_.fetch_add(1, std::memory_order_relaxed) > 1) {
...
void RecursiveCriticalSection::Leave() const RTC_UNLOCK_FUNCTION() {
  ...
  if (lock_queue_.fetch_sub(1, std::memory_order_acq_rel) > 0 && !recursion_)
