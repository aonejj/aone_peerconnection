...
#include "../../rtc_pc_src/RTCInvocableMessageHandler.h"
...
// static
Thread* Thread::Current(void* rtc_thread_manager) {
	ThreadManager* manager = ((RTCThreadManagerInterface*)rtc_thread_manager)->Instance();
...
void* ThreadManager::GetRTCThreadMananger() {
	return _rtc_thread_manager;
}
...
Thread* ThreadManager::WrapCurrentThread() {
  ...
  if (nullptr == result) {
    result = new Thread(CreateDefaultSocketServer(), (void*)((RTCThreadManagerInterface*)GetRTCThreadMananger()));
...
Thread::ScopedDisallowBlockingCalls::ScopedDisallowBlockingCalls(void* rtc_thread_manager)
	: thread_(Thread::Current(rtc_thread_manager)),
...
Thread::ScopedCountBlockingCalls::ScopedCountBlockingCalls(
	std::function<void(uint32_t, uint32_t)> callback, void* rtc_thread_manager)
	: thread_(Thread::Current(rtc_thread_manager)),
...
Thread::Thread(SocketServer* ss, void* rtc_thread_manager
	) : Thread(ss, /*do_init=*/true,
	rtc_thread_manager
	) {
}
...
void Thread::DoInit() {
  ...
  RTC_DCHECK(_rtc_thread_manager != nullptr);
  ((RTCThreadManagerInterface*)_rtc_thread_manager)->Instance()->AddInternal(this);
}
...
void Thread::DoDestroy() {
  ...
  RTC_DCHECK(_rtc_thread_manager != nullptr);
  ((RTCThreadManagerInterface*)_rtc_thread_manager)->Instance()->RemoveInternal(this);
...
bool Thread::IsCurrent() const {
	RTC_DCHECK(_rtc_thread_manager != nullptr);
	return ((RTCThreadManagerInterface*)_rtc_thread_manager)->Instance()->CurrentThread() == this;
}

std::unique_ptr<Thread> Thread::CreateWithSocketServer(
	void* rtc_thread_manager
	) {
  return std::unique_ptr<Thread>(new Thread(CreateDefaultSocketServer(), rtc_thread_manager));
}
...
std::unique_ptr<Thread> Thread::Create(
  ...
  return std::unique_ptr<Thread>(
      new Thread(std::unique_ptr<SocketServer>(new NullSocketServer()), rtc_thread_manager));
}
...
bool Thread::Start() {
  ...
  RTC_DCHECK(_rtc_thread_manager != nullptr);
  ((RTCThreadManagerInterface*)_rtc_thread_manager)->Instance();
...
bool Thread::WrapCurrent() {
  RTC_DCHECK(_rtc_thread_manager != nullptr);
  return WrapCurrentWithThreadManager(((RTCThreadManagerInterface*)_rtc_thread_manager)->Instance(), true);
}

void Thread::UnwrapCurrent() {
  // Clears the platform-specific thread-specific storage.
	RTC_DCHECK(_rtc_thread_manager != nullptr);
	((RTCThreadManagerInterface*)_rtc_thread_manager)->Instance()->SetCurrentThread(nullptr);
...
void Thread::SafeWrapCurrent() {
  RTC_DCHECK(_rtc_thread_manager != nullptr);
  WrapCurrentWithThreadManager(((RTCThreadManagerInterface*)_rtc_thread_manager)->Instance(), false);
}
...
void Thread::Join() {
  ...
  RTC_DCHECK(_rtc_thread_manager != nullptr);
  if (Current(_rtc_thread_manager) && !Current(_rtc_thread_manager)->blocking_calls_allowed_) {
...
void* Thread::PreRun(void* pv) {
  ...
  RTC_DCHECK(thread->_rtc_thread_manager != nullptr);
  ((RTCThreadManagerInterface*)(thread->_rtc_thread_manager))->Instance()->SetCurrentThread(thread);
  ...
  RTC_DCHECK(thread->_rtc_thread_manager != nullptr);
  ((RTCThreadManagerInterface*)(thread->_rtc_thread_manager))->Instance()->SetCurrentThread(nullptr);
...
void Thread::Send(const Location& posted_from,
  ...
  RTC_DCHECK(_rtc_thread_manager != nullptr);
  Thread* current_thread = Thread::Current(_rtc_thread_manager);
  ...
  if (current_thread) {
	...
	RTC_DCHECK(_rtc_thread_manager != nullptr);
	((RTCThreadManagerInterface*)_rtc_thread_manager)->Instance()->RegisterSendAndCheckForCycles(current_thread,
																	this);
  }
#endif
...
void Thread::PostTask(absl::AnyInvocable<void() &&> task) {
  ...
  // takes it with ownership.
	Post(RTC_FROM_HERE, (RTCAnyInvocableMessageHandler*)((RTCThreadManagerInterface*)_rtc_thread_manager)->GetAnyInvocableMessageHandler(),
		/*id=*/0, new RTCAnyInvocableMessage(std::move(task)));
}
...
void Thread::PostDelayedHighPrecisionTask(absl::AnyInvocable<void() &&> task,
  ...
  PostDelayed(RTC_FROM_HERE, delay_ms, (RTCAnyInvocableMessageHandler*)((RTCThreadManagerInterface*)_rtc_thread_manager)->GetAnyInvocableMessageHandler(),
	  /*id=*/0, new RTCAnyInvocableMessage(std::move(task)));
}
...
AutoThread::AutoThread()
    : Thread(CreateDefaultSocketServer(), /*do_init=*/false,
	 nullptr		// TODO... thinking... Intentional crash
	) {
	RTC_DCHECK(0);	//TODO... thinking... Intentional crash
}
...
AutoSocketServerThread::AutoSocketServerThread(SocketServer* ss)
  ...
  // TODO... thinking... Intentional crash
  RTC_DCHECK(0);
}
