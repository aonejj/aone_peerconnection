...
class RTC_EXPORT MessageHandlerAutoCleanup : public MessageHandler {
 public:
  ...
  void SetRTCThreadManager(RTCThreadManagerInterface* rtc_thread_manager) { _rtc_thread_manager = rtc_thread_manager; }

 protected:
  MessageHandlerAutoCleanup();

 protected:
  RTCThreadManagerInterface* _rtc_thread_manager = nullptr;
