...
void P2PTransportChannel::OnCandidateError(
	...
	RTC_DCHECK(network_thread_ == rtc::Thread::Current(_rtc_thread_manager));
...
void P2PTransportChannel::OnCandidateResolved(
  ...
  network_thread_->PostTask(
	  SafeTask(task_safety_.flag(), [resolver]() { resolver->Destroy(false); }));
}
...
void P2PTransportChannel::CheckAndPing() {
  ...
  network_thread_->PostDelayedTask(
	  SafeTask(task_safety_.flag(), [this]() { CheckAndPing(); }), webrtc::TimeDelta::Millis(delay));
}
