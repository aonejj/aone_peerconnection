...
static std::string _debug_loss_state_to_string(LossBasedState &state) {
	switch (state) {
	case LossBasedState::kIncreasing:
		return std::string("kIncreasing");
	case LossBasedState::kDecreasing:
		return std::string("kDecreasing");
	case LossBasedState::kDelayBasedEstimate:
		return std::string("kDelayBasedEstimate");
	}
...
absl::optional<LossBasedBweV2::Config> LossBasedBweV2::CreateConfig(
  ...
  config->bandwidth_rampup_upper_bound_factor = 1000000.0;
  config->rampup_acceleration_max_factor = 0.0;
  config->rampup_acceleration_maxout_time = TimeDelta::Seconds(60);
  config->candidate_factors = { 1.02, 1.0, 0.95 };
  config->higher_bandwidth_bias_factor = 0.0002;
  config->higher_log_bandwidth_bias_factor = 0.02;
  config->inherent_loss_lower_bound = 1.0e-3;
  config->loss_threshold_of_high_bandwidth_preference = 0.15;
  config->bandwidth_preference_smoothing_factor = 0.002;
  config->inherent_loss_upper_bound_bandwidth_balance = DataRate::KilobitsPerSec(75.0);
  config->inherent_loss_upper_bound_offset = 0.05;
  config->initial_inherent_loss_estimate = 0.01;
  config->newton_iterations = 1;
  config->newton_step_size = 0.75;
  config->append_acknowledged_rate_candidate = true;
  config->append_delay_based_estimate_candidate = true;
  config->observation_duration_lower_bound = TimeDelta::Millis(250);
  ...
  config->instant_upper_bound_bandwidth_balance = DataRate::KilobitsPerSec(75.0);
  config->instant_upper_bound_loss_offset = 0.05;
  config->temporal_weight_factor = 0.9;
  config->bandwidth_backoff_lower_bound_factor = 1.0;
  config->trendline_integration_enabled = false;
  config->trendline_observations_window_size = 20;
  config->max_increase_factor = 1.3;
  config->delayed_increase_window = TimeDelta::Millis(300);
  config->use_acked_bitrate_only_when_overusing = false;
  config->not_increase_if_inherent_loss_less_than_average_loss = true;
  config->high_loss_rate_threshold = 1.0;
  config->bandwidth_cap_at_high_loss_rate = DataRate::KilobitsPerSec(500.0);
  config->slope_of_bwe_high_loss_func = 1000;
  config->probe_integration_enabled = false;
  config->probe_expiration = TimeDelta::Seconds(10);
  config->bound_by_upper_link_capacity_when_loss_limited = true;
  config->not_use_acked_rate_in_alr = false;
