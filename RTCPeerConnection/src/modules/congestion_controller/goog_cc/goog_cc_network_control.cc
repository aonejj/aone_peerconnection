...
//    event_log_(config.event_log),												
      ...
      use_min_allocatable_as_lower_bound_(true /*IsNotDisabled(key_value_config_, "WebRTC-Bwe-MinAllocAsLowerBound") */),
      ignore_probes_lower_than_network_estimate_(true /* IsNotDisabled(
          key_value_config_,
      ...
      rate_control_settings_(
          RateControlSettings::ParseFromKeyValueConfig(nullptr /* key_value_config_ */ )),
      loss_based_stable_rate_(true
          /* IsEnabled(key_value_config_, "WebRTC-Bwe-LossBasedStableRate") */),
      probe_controller_(
          new ProbeController(nullptr, nullptr /* key_value_config_, config.event_log */)),
      congestion_window_pushback_controller_(
          rate_control_settings_.UseCongestionWindowPushback()
              ? std::make_unique<CongestionWindowPushbackController>( nullptr
                    /* key_value_config_ */)
              : nullptr),
      bandwidth_estimation_(
	  std::make_unique<SendSideBandwidthEstimation>(nullptr/* key_value_config_ */, event_log_)),
      alr_detector_(
          std::make_unique<AlrDetector>( nullptr, nullptr /* key_value_config_, config.event_log) */)),
      probe_bitrate_estimator_(new ProbeBitrateEstimator(nullptr /* config.event_log */)),
      ...
      delay_based_bwe_(new DelayBasedBwe(nullptr /* key_value_config_ */,
										 event_log_,
                                         network_state_predictor_.get())),
      acknowledged_bitrate_estimator_(
          AcknowledgedBitrateEstimatorInterface::Create(nullptr /* key_value_config_ */)),
...
NetworkControlUpdate GoogCcNetworkController::OnNetworkRouteChange(
  ...
  delay_based_bwe_.reset(new DelayBasedBwe( nullptr /* key_value_config_ */, event_log_,
                                           network_state_predictor_.get()));
...
//////////////////////////////////////////////////////////////////////////
// kimi for debug
static std::string _input_bw_state_2_string(BandwidthUsage bw) {
	switch (bw) {
