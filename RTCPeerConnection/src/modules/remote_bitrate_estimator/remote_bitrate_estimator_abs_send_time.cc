...
absl::optional<DataRate> OptionalRateFromOptionalBps(
  ...
  if (bitrate_bps) {
//	  RTC_LOG(LS_INFO) << "rbe_trace OptionalRateFromOptionalBps bitrate_bps " << *bitrate_bps;
    return DataRate::BitsPerSec(*bitrate_bps);
  } else {
//	  RTC_LOG(LS_INFO) << "rbe_trace OptionalRateFromOptionalBps bitrate_bps absl::nullopt";
    return absl::nullopt;
  }
}
}  // namespace
...
RemoteBitrateEstimatorAbsSendTime::ProcessClusters(int64_t now_ms) {
  ...
  if (best_it != clusters.end()) {
    ...
    if (IsBitrateImproving(probe_bitrate_bps)) {
      RTC_LOG(LS_INFO) << "rbe_trace Probe successful, sent at "
...
void RemoteBitrateEstimatorAbsSendTime::IncomingPacket(const RtpPacketReceived& packet){
  ...
  // add kimi
  if (packet._is_video) {
	  (const_cast<RtpPacketReceived*>(&packet))->_incoming_rate = _incoming_bitrate;
	  (const_cast<RtpPacketReceived*>(&packet))->_bw = (uint16_t)_prev_bw;
  }
}

// add kimi
static std::string _bw_to_string_debug(BandwidthUsage bw) {
	switch (bw) {
...
void RemoteBitrateEstimatorAbsSendTime::IncomingPacketInfo(
  ...
  BandwidthUsage cur_bw = BandwidthUsage::kLast;	// add kimi
  ...
  std::vector<uint32_t> ssrcs;
  {
    ...
    if (inter_arrival_->ComputeDeltas(timestamp, arrival_time_ms, now_ms,
	  ...
	  cur_bw = detector_.State();	// add kimi...
    }
	...
	//////////////////////////////////////////////////////////////////////////
	// add kimi
	if (cur_bw != BandwidthUsage::kLast) {
		if (!update_estimate && _prev_bw != cur_bw) {
			update_estimate = true;
		}
		_prev_bw = cur_bw;
	}
    ...
    if (update_estimate) {
	  ...
	  _incoming_bitrate = *(incoming_bitrate_.Rate(arrival_time_ms));
...
void RemoteBitrateEstimatorAbsSendTime::OnRttUpdate(int64_t avg_rtt_ms,
                                                    int64_t max_rtt_ms) {
  MutexLock lock(&mutex_);
  _avg_rtt_ms = avg_rtt_ms;	// add kimi... for debug
