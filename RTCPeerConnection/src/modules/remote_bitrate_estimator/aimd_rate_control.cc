  ...
  switch (rate_control_state_) {
    ...
    case RateControlState::kRcDecrease: {
		...
		if (decreased_bitrate < current_bitrate_) {
			...
			RTC_LOG(LS_INFO) << "[" << this << "] twcc_trace aimd_trace AimdRateControl::ChangeBitrate kRcDecrease new_bitrate " << (*new_bitrate).bps();
		}
...
// add kimi for debug
std::string AimdRateControl::_input_bw_state_2_string(BandwidthUsage bw) {
	switch (bw) {
...
// add kimi for debug
std::string AimdRateControl::_rate_control_state_2_string(RateControlState rs) {
	switch (rs) {
	case RateControlState::kRcHold:
		return "kRcHold";
	case RateControlState::kRcIncrease:
