...
void PacketSequencer::Sequence(RtpPacketToSend& packet) {
  if (packet.Ssrc() == media_ssrc_) {
    ...
    packet.SetSequenceNumber(rtx_sequence_number_++);	
	  ...
	  RTC_LOG(LS_ERROR) << "Unexpected ssrc " << packet.Ssrc();
	  RTC_DCHECK(0);
  }
}
...
void PacketSequencer::UpdateLastPacketState(const RtpPacketToSend& packet) {
  ...
  last_capture_time_ = Timestamp::Millis(packet.capture_time_ms());
}
...
void PacketSequencer::PopulatePaddingFields(RtpPacketToSend& packet) {
  if (packet.Ssrc() == media_ssrc_) {
    ...
    packet.set_capture_time_ms(last_capture_time_.ms());
  ...
  if (last_timestamp_time_ > Timestamp::Zero()) {
	...
	if (packet.capture_time_ms() > 0ll) {
      packet.set_capture_time_ms(packet.capture_time_ms() + since_last_media.ms());
    }
  }
}
...
bool PacketSequencer::CanSendPaddingOnMediaSsrc() const {
  if (last_payload_type_ == -1) {
	  RTC_LOG(LS_INFO) << "xxxx last_payload_type_ == -1 return false";
    return false;
  }
  ...
  if (require_marker_before_media_padding_ && !last_packet_marker_bit_) {
	  RTC_LOG(LS_INFO) << "require_marker_before_media_padding_ && !last_packet_marker_bit_";
    return false;
  }
