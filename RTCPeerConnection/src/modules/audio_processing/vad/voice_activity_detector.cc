  ...
  //////////////////////////////////////////////////////////////////////////
	// kimi... for debug
	  ...
	  for (int i = 0; i < length; ++i) {
		  if (std::abs(resampled_ptr[i]) > max_abs)
			  max_abs = std::abs(resampled_ptr[i]);
	  }
 //////////////////////////////////////////////////////////////////////////
...
// add kimi
void VoiceActivityDetector::ProcessChunkExt(const int16_t* audio, size_t length, int sample_rate_hz,
										    bool &isValid, float& lastVoiceProbability) {
	ProcessChunk(audio, length, sample_rate_hz);

	if (features_.num_frames > 0) {
		isValid = true;
		lastVoiceProbability = last_voice_probability_;
	}
	else {
		isValid = false;
	}
