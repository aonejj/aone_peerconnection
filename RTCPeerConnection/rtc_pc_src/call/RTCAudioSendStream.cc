...
RTCAudioSendStream::Stats::Stats() = default;
RTCAudioSendStream::Stats::~Stats() = default;

RTCAudioSendStream::Config::Config(Transport* send_transport)
	: send_transport(send_transport) {}

RTCAudioSendStream::Config::~Config() = default;

std::string RTCAudioSendStream::Config::ToString() const {
	char buf[1024];
...
RTCAudioSendStream::Config::Rtp::Rtp() = default;

RTCAudioSendStream::Config::Rtp::~Rtp() = default;

std::string RTCAudioSendStream::Config::Rtp::ToString() const {
	char buf[1024];
	rtc::SimpleStringBuilder ss(buf);
	ss << "{ssrc: " << ssrc;
	if (!rid.empty()) {
...
RTCAudioSendStream::Config::SendCodecSpec::SendCodecSpec(
	int payload_type,
	const SdpAudioFormat& format)
	: payload_type(payload_type), format(format) {}

RTCAudioSendStream::Config::SendCodecSpec::~SendCodecSpec() = default;

std::string RTCAudioSendStream::Config::SendCodecSpec::ToString() const {
	char buf[1024];
	rtc::SimpleStringBuilder ss(buf);
...
bool RTCAudioSendStream::Config::SendCodecSpec::operator==(
	const RTCAudioSendStream::Config::SendCodecSpec& rhs) const {
