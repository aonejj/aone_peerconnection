...
std::unique_ptr<RTCRtpRtcpRecvImpl> CreateRtpRtcpModule(
	...
	const RTCFlexFecReceiveStreamImpl::Config& config,
	...
	return RTCRtpRtcpRecvImpl::Create(configuration);
}

}  // namespace

RTCFlexFecReceiveStreamImpl::RTCFlexFecReceiveStreamImpl(
	Clock* clock,
	...
	config_,
	rtt_stats)) {
	RTC_LOG(LS_INFO) << "RTCFlexFecReceiveStreamImpl: " << config_.ToString();

	// RTCP reporting.
	rtp_rtcp_->SetRTCPStatus(config_.rtcp_mode);
	//  process_thread_->RegisterModule(rtp_rtcp_.get(), RTC_FROM_HERE);	// kimi... disable
...
void RTCFlexFecReceiveStreamImpl::OnRtpPacket(const RtpPacketReceived& packet) {
	if (!receiver_)
		return;
...
// stats for FlexFEC.
FlexfecReceiveStream::Stats RTCFlexFecReceiveStreamImpl::GetStats() const {
	return FlexfecReceiveStream::Stats();
}

const FlexfecReceiveStream::Config& RTCFlexFecReceiveStreamImpl::GetConfig()
	const {
	return config_;
}
