...
//
// author : kimi
// desc   : replace to  /pc/channel_manager.cc
//
//////////////////////////////////////////////////////////////////////////
...
RTCChannelManager::RTCChannelManager(
	...
	: media_engine_(std::move(media_engine)),
	  ...
	  main_thread_(rtc::Thread::Current(rtc_thread_manager)),
...
bool RTCChannelManager::SetVideoRtxEnabled(bool enable) {
...
void RTCChannelManager::GetSupportedAudioSendCodecs(
...
void RTCChannelManager::GetSupportedAudioReceiveCodecs(
						std::vector<AudioCodec>* codecs) const {
	if (!media_engine_) {
		return;
	}
...
void RTCChannelManager::GetSupportedVideoSendCodecs(
...
void RTCChannelManager::GetSupportedVideoReceiveCodecs(
						std::vector<VideoCodec>* codecs) const {
	if (!media_engine_) {
		return;
	}
...
void RTCChannelManager::GetSupportedDataCodecs(
...
RtpHeaderExtensions RTCChannelManager::GetDefaultEnabledAudioRtpHeaderExtensions()
	const {
	if (!media_engine_)
		return{};
...
RTCChannelManager::GetSupportedAudioRtpHeaderExtensions() const {
	if (!media_engine_)
		return{};
	return media_engine_->voice().GetRtpHeaderExtensions();
}

RtpHeaderExtensions RTCChannelManager::GetDefaultEnabledVideoRtpHeaderExtensions()
	const {
	if (!media_engine_)
		return{};
...
std::vector<webrtc::RtpHeaderExtensionCapability>
RTCChannelManager::GetSupportedVideoRtpHeaderExtensions() const {
	if (!media_engine_)
		return{};
	return media_engine_->video().GetRtpHeaderExtensions();
}

void RTCChannelManager::Terminate() {
	RTC_DCHECK(initialized_);
	if (!initialized_) {
		return;
	}
...
RTCVoiceChannel* RTCChannelManager::CreateVoiceChannel(
	webrtc::RTCCall* call,		
	...
	if (!worker_thread_->IsCurrent()) {
		return worker_thread_->Invoke<RTCVoiceChannel*>(RTC_FROM_HERE, [&] {
	...
	RTC_DCHECK(initialized_);
	if (!media_engine_) {
		RTC_LOG(LS_VERBOSE) << "RTCChannelManager::CreateVoiceChannel media_engine_ == nullptr";
		return nullptr;
	}
	...
	auto voice_channel = std::make_unique<RTCVoiceChannel>(
	...
	RTCVoiceChannel* voice_channel_ptr = voice_channel.get();
	voice_channels_.push_back(std::move(voice_channel));
	return voice_channel_ptr;
}

void RTCChannelManager::DestroyVoiceChannel(RTCVoiceChannel* voice_channel) {
	if (!voice_channel) {
		return;
	}
	if (!worker_thread_->IsCurrent()) {
	...
	auto it = absl::c_find_if(voice_channels_,
		[&](const std::unique_ptr<RTCVoiceChannel>& p) {
...
RTCVideoChannel* RTCChannelManager::CreateVideoChannel(
	webrtc::RTCCall* call,
	const cricket::MediaConfig& media_config,
	webrtc::RtpTransportInternal* rtp_transport,
	rtc::Thread* signaling_thread,
	...
	// thread.
	if (!worker_thread_->IsCurrent()) {
		return worker_thread_->Invoke<RTCVideoChannel*>(RTC_FROM_HERE, [&] {
	...
	auto video_channel = std::make_unique<RTCVideoChannel>(
		worker_thread_, network_thread_, signaling_thread,
		absl::WrapUnique(media_channel), content_name, srtp_required,
		crypto_options, ssrc_generator,
		_rtc_thread_manager
	...
	RTCVideoChannel* video_channel_ptr = video_channel.get();
	video_channels_.push_back(std::move(video_channel));
	return video_channel_ptr;
}

void RTCChannelManager::DestroyVideoChannel(RTCVideoChannel* video_channel) {
	if (!video_channel) {
		return;
	}
	if (!worker_thread_->IsCurrent()) {
	...
	auto it = absl::c_find_if(video_channels_,
		[&](const std::unique_ptr<RTCVideoChannel>& p) {
...
RTCRtpDataChannel* RTCChannelManager::CreateRtpDataChannel(
	const cricket::MediaConfig& media_config,
	webrtc::RtpTransportInternal* rtp_transport,
	rtc::Thread* signaling_thread,
	const std::string& content_name,
	...
	rtc::UniqueRandomIdGenerator* ssrc_generator) {
	if (!worker_thread_->IsCurrent()) {
		return worker_thread_->Invoke<RTCRtpDataChannel*>(RTC_FROM_HERE, [&] {
	...
	auto data_channel = std::make_unique<RTCRtpDataChannel>(
		worker_thread_, network_thread_, signaling_thread,
		absl::WrapUnique(media_channel), content_name, srtp_required,
		crypto_options, ssrc_generator,
		_rtc_thread_manager
	...
	RTCRtpDataChannel* data_channel_ptr = data_channel.get();
	data_channels_.push_back(std::move(data_channel));
	return data_channel_ptr;
}

void RTCChannelManager::DestroyRtpDataChannel(RTCRtpDataChannel* data_channel) {
	if (!data_channel) {
		return;
	}
	if (!worker_thread_->IsCurrent()) {
	...
	auto it = absl::c_find_if(data_channels_,
		[&](const std::unique_ptr<RTCRtpDataChannel>& p) {
