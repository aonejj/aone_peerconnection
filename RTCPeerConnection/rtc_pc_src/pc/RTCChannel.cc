...
RTCBaseChannel::RTCBaseChannel(rtc::Thread* worker_thread,
	...
	: worker_thread_(worker_thread),
	  ...
	  alive_(webrtc::PendingTaskSafetyFlag::Create()),
	  ...
	  _rtc_thread_manager(rtc_thread_manager)
{
	...
	SetRTCThreadManager(_rtc_thread_manager);
	RTC_LOG(LS_INFO) << "Created channel: " << ToString();
}

RTCBaseChannel::~RTCBaseChannel() {
	RTC_DCHECK_RUN_ON(worker_thread_);
...
std::string RTCBaseChannel::ToString() const {
...
bool RTCBaseChannel::ConnectToRtpTransport() {
	...
	rtp_transport_->SignalReadyToSend.connect(
		this, &RTCBaseChannel::OnTransportReadyToSend);
	rtp_transport_->SignalNetworkRouteChanged.connect(
		this, &RTCBaseChannel::OnNetworkRouteChanged);
	rtp_transport_->SignalWritableState.connect(this,
		&RTCBaseChannel::OnWritableState);
	rtp_transport_->SignalSentPacket.connect(this,
		&RTCBaseChannel::SignalSentPacket_n);
	return true;
}

void RTCBaseChannel::DisconnectFromRtpTransport() {
	RTC_DCHECK_RUN_ON(network_thread());
	RTC_DCHECK(rtp_transport_);
...
void RTCBaseChannel::Init_w(webrtc::RtpTransportInternal* rtp_transport) {
...
bool RTCBaseChannel::SetRtpTransport(webrtc::RtpTransportInternal* rtp_transport) {
...
bool RTCBaseChannel::Enable(bool enable) {
...
bool RTCBaseChannel::SetLocalContent(const MediaContentDescription* content,
									 webrtc::SdpType type,
...
bool RTCBaseChannel::SetRemoteContent(const MediaContentDescription* content,
									  webrtc::SdpType type,
									  std::string* error_desc) {
	return InvokeOnWorker<bool>(RTC_FROM_HERE, [this, content, type, error_desc] {
...
bool RTCBaseChannel::SetPayloadTypeDemuxingEnabled(bool enabled) {
...
bool RTCBaseChannel::IsReadyToReceiveMedia_w() const {
	// Receive data if we are enabled and have local content,
	return enabled() &&
		webrtc::RtpTransceiverDirectionHasRecv(local_content_direction_);
}

bool RTCBaseChannel::IsReadyToSendMedia_w() const {
...
bool RTCBaseChannel::IsReadyToSendMedia_n() const {
...
bool RTCBaseChannel::SendPacket(rtc::CopyOnWriteBuffer* packet,
								const rtc::PacketOptions& options) {
	return SendPacket(false, packet, options);
}

bool RTCBaseChannel::SendRtcp(rtc::CopyOnWriteBuffer* packet,
							  const rtc::PacketOptions& options) {
	return SendPacket(true, packet, options);
}

int RTCBaseChannel::SetOption(SocketType type,
...
int RTCBaseChannel::SetOption_n(SocketType type,
	rtc::Socket::Option opt,
	int value) {
	RTC_DCHECK(rtp_transport_);
	switch (type) {
...
void RTCBaseChannel::OnWritableState(bool writable) {
	RTC_DCHECK_RUN_ON(network_thread());
	if (writable) {
		ChannelWritable_n();
	}
...
void RTCBaseChannel::OnNetworkRouteChanged(
	...
	worker_thread_->PostTask(webrtc::SafeTask(alive_, [this, new_route] {
		RTC_DCHECK_RUN_ON(worker_thread());
		media_channel_->OnNetworkRouteChanged(transport_name_, new_route);
	}));
}

sigslot::signal1<ChannelInterface*>& RTCBaseChannel::SignalFirstPacketReceived() {
	RTC_DCHECK_RUN_ON(signaling_thread_);
	return SignalFirstPacketReceived_;
}

sigslot::signal1<const rtc::SentPacket&>& RTCBaseChannel::SignalSentPacket() {
...
void RTCBaseChannel::OnTransportReadyToSend(bool ready) {
	worker_thread_->PostTask(webrtc::SafeTask(alive_, [this, ready] {
		RTC_DCHECK_RUN_ON(worker_thread());
		media_channel_->OnReadyToSend(ready);
	}));
}

bool RTCBaseChannel::SendPacket(bool rtcp,
	rtc::CopyOnWriteBuffer* packet,
	const rtc::PacketOptions& options) {
...
void RTCBaseChannel::OnRtpPacket(const webrtc::RtpPacketReceived& parsed_packet) {
...
void RTCBaseChannel::UpdateRtpHeaderExtensionMap(
...
bool RTCBaseChannel::RegisterRtpDemuxerSink_w() {
...
bool RTCBaseChannel::RegisterRtpDemuxerSink_n() {
	RTC_DCHECK(rtp_transport_);
...
void RTCBaseChannel::EnableMedia_w() {
	RTC_DCHECK(worker_thread_ == rtc::Thread::Current(_rtc_thread_manager));
	if (enabled_)
		return;
...
void RTCBaseChannel::DisableMedia_w() {
	RTC_DCHECK(worker_thread_ == rtc::Thread::Current(_rtc_thread_manager));
	if (!enabled_)
		return;
...
void RTCBaseChannel::UpdateWritableState_n() {
...
void RTCBaseChannel::ChannelWritable_n() {
	if (writable_) {
		return;
	}
...
void RTCBaseChannel::ChannelNotWritable_n() {
	if (!writable_)
		return;
...
bool RTCBaseChannel::AddRecvStream_w(const StreamParams& sp) {
	return media_channel()->AddRecvStream(sp);
}

bool RTCBaseChannel::RemoveRecvStream_w(uint32_t ssrc) {
	return media_channel()->RemoveRecvStream(ssrc);
}

void RTCBaseChannel::ResetUnsignaledRecvStream_w() {
	RTC_DCHECK(worker_thread_ == rtc::Thread::Current(_rtc_thread_manager));
	media_channel()->ResetUnsignaledRecvStream();
}

bool RTCBaseChannel::SetPayloadTypeDemuxingEnabled_w(bool enabled) {
...
bool RTCBaseChannel::UpdateLocalStreams_w(const std::vector<StreamParams>& streams,
										  webrtc::SdpType type,
										  std::string* error_desc) {
...
bool RTCBaseChannel::UpdateRemoteStreams_w(
	...
	for (const StreamParams& old_stream : remote_streams_) {
		...
		if (!old_stream.has_ssrcs() && !HasStreamWithNoSsrcs(streams)) {
			...
			if (media_type() == MEDIA_TYPE_AUDIO || media_type() == MEDIA_TYPE_DATA || 
				!old_stream.has_ssrc_group(kSimSsrcGroupSemantics)) {
				...
				// SIM stream
				const SsrcGroup* ssrc_sim_group = old_stream.get_ssrc_group(kSimSsrcGroupSemantics);
				for (uint32_t ssrc : ssrc_sim_group->ssrcs) {
					if (RemoveRecvStream_w(ssrc)) {
						RTC_LOG(LS_INFO) << "Remove simulcast remote ssrc: " << ssrc
							<< " from " << ToString() << ".";
					}
				}
			}
	...
	for (const StreamParams& new_stream : streams) {
		...
		if ((!new_stream.has_ssrcs() && !HasStreamWithNoSsrcs(remote_streams_)) ||
			...
			if (media_type() == MEDIA_TYPE_AUDIO || media_type() == MEDIA_TYPE_DATA || 
				!new_stream.has_ssrc_group(kSimSsrcGroupSemantics)) {
				...
				const SsrcGroup* ssrc_sim_group = new_stream.get_ssrc_group(kSimSsrcGroupSemantics);
				for (uint32_t ssrc : ssrc_sim_group->ssrcs) {
					StreamParams sp = StreamParams::CreateLegacy(ssrc);
					uint32_t fid_ssrc;
					new_stream.GetFidSsrc(ssrc, &fid_ssrc);
					sp.AddFidSsrc(ssrc, fid_ssrc);
					sp.set_stream_ids(new_stream.stream_ids());
					sp.id = new_stream.id;
					sp.cname = new_stream.cname;
...
RtpHeaderExtensions RTCBaseChannel::GetFilteredRtpHeaderExtensions(
							const RtpHeaderExtensions& extensions) {
...
void RTCBaseChannel::OnMessage(rtc::Message* pmsg) {
...
void RTCBaseChannel::MaybeAddHandledPayloadType(int payload_type) {
...
void RTCBaseChannel::ClearHandledPayloadTypes() {
	demuxer_criteria_.payload_types.clear();
	payload_types_.clear();
}

void RTCBaseChannel::FlushRtcpMessages_n() {
...
void RTCBaseChannel::SignalSentPacket_n(const rtc::SentPacket& sent_packet) {
	worker_thread_->PostTask(webrtc::SafeTask(alive_, [this, sent_packet] {
		RTC_DCHECK_RUN_ON(worker_thread());
		SignalSentPacket()(sent_packet);
	}));
}

void RTCBaseChannel::SetNegotiatedHeaderExtensions_w(
	const RtpHeaderExtensions& extensions) {
	RTC_DCHECK_RUN_ON(worker_thread());
...
RtpHeaderExtensions RTCBaseChannel::GetNegotiatedRtpHeaderExtensions() const {
	RTC_DCHECK_RUN_ON(signaling_thread());
	webrtc::MutexLock lock(&negotiated_header_extensions_lock_);
	return negotiated_header_extensions_;
}

void RTCBaseChannel::UpdateMediaSendRecvState() {
	RTC_DCHECK_RUN_ON(network_thread());
	worker_thread_->PostTask(
		webrtc::SafeTask(alive_, [this] { UpdateMediaSendRecvState_w(); }));
}

RTCVoiceChannel::RTCVoiceChannel(rtc::Thread* worker_thread,
								 rtc::Thread* network_thread,
								 rtc::Thread* signaling_thread,
...
RTCVoiceChannel::~RTCVoiceChannel() {
	// this can't be done in the base class, since it calls a virtual
	DisableMedia_w();
	Deinit();
}

void RTCVoiceChannel::Init_w(webrtc::RtpTransportInternal* rtp_transport) {
	RTCBaseChannel::Init_w(rtp_transport);
}

void RTCVoiceChannel::UpdateMediaSendRecvState_w() {
...
bool RTCVoiceChannel::SetLocalContent_w(const MediaContentDescription* content,
										webrtc::SdpType type,
										std::string* error_desc) {
	RTC_DCHECK_RUN_ON(worker_thread());
...
bool RTCVoiceChannel::SetRemoteContent_w(const MediaContentDescription* content,
										 webrtc::SdpType type,
										 std::string* error_desc) {
	RTC_DCHECK_RUN_ON(worker_thread());
...
//////////////////////////////////////////////////////////////////////////
// RTCVideoChannel
RTCVideoChannel::RTCVideoChannel(rtc::Thread* worker_thread,
								 rtc::Thread* network_thread,
								 rtc::Thread* signaling_thread,
...
RTCVideoChannel::~RTCVideoChannel() {
	// this can't be done in the base class, since it calls a virtual
	RTC_LOG(LS_VERBOSE) << "RTCVideoChannel::~RTCVideoChannel";
	DisableMedia_w();
	Deinit();
}

void RTCVideoChannel::UpdateMediaSendRecvState_w() {
	// Send outgoing data if we're the active call, we have the remote content,
	// and we have had some form of connectivity.
	RTC_DCHECK_RUN_ON(worker_thread());
	bool send = IsReadyToSendMedia_w();
...
bool RTCVideoChannel::SetLocalContent_w(const MediaContentDescription* content,
										webrtc::SdpType type,
										std::string* error_desc) {
	RTC_DCHECK_RUN_ON(worker_thread());
	...
	if (type == webrtc::SdpType::kAnswer || type == webrtc::SdpType::kPrAnswer) {
...
bool RTCVideoChannel::SetRemoteContent_w(const MediaContentDescription* content,
										 webrtc::SdpType type,
										 std::string* error_desc) {
	RTC_DCHECK_RUN_ON(worker_thread());
...
//////////////////////////////////////////////////////////////////////////
// RTCRtpDataChannel
RTCRtpDataChannel::RTCRtpDataChannel(rtc::Thread* worker_thread,
									 rtc::Thread* network_thread,
									 rtc::Thread* signaling_thread,
...
RTCRtpDataChannel::~RTCRtpDataChannel() {
	// this can't be done in the base class, since it calls a virtual
	DisableMedia_w();
	Deinit();
}

void RTCRtpDataChannel::Init_w(webrtc::RtpTransportInternal* rtp_transport) {
	RTCBaseChannel::Init_w(rtp_transport);
	media_channel()->SignalDataReceived.connect(this,
		&RTCRtpDataChannel::OnDataReceived);
	media_channel()->SignalReadyToSend.connect(
		this, &RTCRtpDataChannel::OnDataChannelReadyToSend);
}

bool RTCRtpDataChannel::SendData(const SendDataParams& params,
...
bool RTCRtpDataChannel::CheckDataChannelTypeFromContent(
...
bool RTCRtpDataChannel::SetLocalContent_w(const MediaContentDescription* content,
										  webrtc::SdpType type,
										  std::string* error_desc) {
	RTC_DCHECK_RUN_ON(worker_thread());
...
bool RTCRtpDataChannel::SetRemoteContent_w(const MediaContentDescription* content,
										   webrtc::SdpType type,
										   std::string* error_desc) {
	RTC_DCHECK_RUN_ON(worker_thread());
...
void RTCRtpDataChannel::UpdateMediaSendRecvState_w() {
	// Render incoming data if we're the active call, and we have the local
	// content. We receive data on the default channel and multiplexed streams.
	RTC_DCHECK_RUN_ON(worker_thread());
	bool recv = IsReadyToReceiveMedia_w();
...
void RTCRtpDataChannel::OnMessage(rtc::Message* pmsg) {
	switch (pmsg->message_id) {
	...
	default:
		RTCBaseChannel::OnMessage(pmsg);
		break;
	}
}

void RTCRtpDataChannel::OnDataReceived(const ReceiveDataParams& params,
	const char* data,
	size_t len) {
...
void RTCRtpDataChannel::OnDataChannelReadyToSend(bool writable) {
