...
//
// author : kimi
// desc   : DataChannelController replace.
//
//////////////////////////////////////////////////////////////////////////
...
bool RTCDataChannelController::HasDataChannels() const {
	RTC_DCHECK_RUN_ON(signaling_thread());
	return !rtp_data_channels_.empty() || !sctp_data_channels_.empty();
}

bool RTCDataChannelController::SendData(const cricket::SendDataParams& params,
...
bool RTCDataChannelController::ConnectDataChannel(
...
void RTCDataChannelController::DisconnectDataChannel(
	RtpDataChannel* webrtc_data_channel) {
	RTC_DCHECK_RUN_ON(signaling_thread());
	if (!rtp_data_channel()) {
		RTC_LOG(LS_ERROR)
...
void RTCDataChannelController::AddSctpDataStream(int sid) {
	if (data_channel_transport()) {
...
void RTCDataChannelController::RemoveSctpDataStream(int sid) {
	if (data_channel_transport()) {
		network_thread()->Invoke<void>(RTC_FROM_HERE, [this, sid] {
			if (data_channel_transport()) {
...
void RTCDataChannelController::OnChannelClosing(int channel_id) {
	RTC_DCHECK_RUN_ON(network_thread());
	data_channel_transport_invoker_.AsyncInvoke<void>(
...
void RTCDataChannelController::OnChannelClosed(int channel_id) {
	RTC_DCHECK_RUN_ON(network_thread());
	data_channel_transport_invoker_.AsyncInvoke<void>(
		RTC_FROM_HERE, signaling_thread(), [this, channel_id] {
		RTC_DCHECK_RUN_ON(signaling_thread());
...
void RTCDataChannelController::OnReadyToSend() {
	RTC_DCHECK_RUN_ON(network_thread());
	data_channel_transport_invoker_.AsyncInvoke<void>(
...
void RTCDataChannelController::OnTransportClosed() {
	RTC_DCHECK_RUN_ON(network_thread());
	data_channel_transport_invoker_.AsyncInvoke<void>(
		RTC_FROM_HERE, signaling_thread(), [this] {
		RTC_DCHECK_RUN_ON(signaling_thread());
...
void RTCDataChannelController::SetupDataChannelTransport_n() {
	RTC_DCHECK_RUN_ON(network_thread());
...
void RTCDataChannelController::TeardownDataChannelTransport_n() {
	RTC_DCHECK_RUN_ON(network_thread());
	if (data_channel_transport()) {
...
void RTCDataChannelController::OnTransportChanged(
...
std::vector<DataChannelStats> RTCDataChannelController::GetDataChannelStats()
	const {
	RTC_DCHECK_RUN_ON(signaling_thread());
...
bool RTCDataChannelController::HandleOpenMessage_s(
...
void RTCDataChannelController::OnDataChannelOpenMessage(
...
RTCDataChannelController::InternalCreateDataChannelWithProxy(
	const std::string& label,
	const InternalDataChannelInit* config) {
	RTC_DCHECK_RUN_ON(signaling_thread());
	if (pc_->IsClosed()) {
...
RTCDataChannelController::InternalCreateRtpDataChannel(
	const std::string& label,
...
RTCDataChannelController::InternalCreateSctpDataChannel(
	const std::string& label,
	const InternalDataChannelInit* config) {
	RTC_DCHECK_RUN_ON(signaling_thread());
	...
	rtc::scoped_refptr<SctpDataChannel> channel(SctpDataChannel::Create(
		this, label, new_config, signaling_thread(), network_thread()	, _rtc_thread_manager));
	if (!channel) {
		sid_allocator_.ReleaseSid(new_config.id);
		return nullptr;
	}
	sctp_data_channels_.push_back(channel);
//	channel->SignalClosed.connect(pc_, &RTCConnection::OnSctpDataChannelClosed);
	pc_->SctpDataChannelConnectClosedSignal(channel.get());

	SignalSctpDataChannelCreated_(channel.get());
	return channel;
}

void RTCDataChannelController::AllocateSctpSids(rtc::SSLRole role) {
	RTC_DCHECK_RUN_ON(signaling_thread());
...
void RTCDataChannelController::OnSctpDataChannelClosed(SctpDataChannel* channel) {
	RTC_DCHECK_RUN_ON(signaling_thread());
...
void RTCDataChannelController::OnTransportChannelClosed() {
	RTC_DCHECK_RUN_ON(signaling_thread());
...
SctpDataChannel* RTCDataChannelController::FindDataChannelBySid(int sid) const {
	RTC_DCHECK_RUN_ON(signaling_thread());
	for (const auto& channel : sctp_data_channels_) {
		if (channel->id() == sid) {
			return channel;
...
void RTCDataChannelController::UpdateLocalRtpDataChannels(
...
void RTCDataChannelController::UpdateRemoteRtpDataChannels(
	const cricket::StreamParamsVec& streams) {
	RTC_DCHECK_RUN_ON(signaling_thread());
...
cricket::DataChannelType RTCDataChannelController::data_channel_type() const {
	// TODO(bugs.webrtc.org/9987): Should be restricted to the signaling thread.
	// RTC_DCHECK_RUN_ON(signaling_thread());
	return data_channel_type_;
}

void RTCDataChannelController::set_data_channel_type(
	cricket::DataChannelType type) {
	RTC_DCHECK_RUN_ON(signaling_thread());
	data_channel_type_ = type;
}

DataChannelTransportInterface* RTCDataChannelController::data_channel_transport()
	const {
...
void RTCDataChannelController::set_data_channel_transport(
...
RTCDataChannelController::rtp_data_channels() const {
	RTC_DCHECK_RUN_ON(signaling_thread());
	return &rtp_data_channels_;
}

void RTCDataChannelController::UpdateClosingRtpDataChannels(
...
void RTCDataChannelController::CreateRemoteRtpDataChannel(const std::string& label,
...
bool RTCDataChannelController::DataChannelSendData(
...
void RTCDataChannelController::NotifyDataChannelsOfTransportCreated() {
	RTC_DCHECK_RUN_ON(network_thread());
	data_channel_transport_invoker_.AsyncInvoke<void>(
		RTC_FROM_HERE, signaling_thread(), [this] {
		RTC_DCHECK_RUN_ON(signaling_thread());
...
rtc::Thread* RTCDataChannelController::network_thread() const {
	return pc_->network_thread();
}
rtc::Thread* RTCDataChannelController::signaling_thread() const {
