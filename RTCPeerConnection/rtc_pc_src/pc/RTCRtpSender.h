...
//
// author : kimi
// desc   : replace to  /pc/rtp_sender.h
//
//////////////////////////////////////////////////////////////////////////

#ifndef __RTC_PC_RTP_SENDER_H__
#define __RTC_PC_RTP_SENDER_H__
...
class RTCRtpSenderInternal : public RTCRtpSenderInterface {
public:
...
class RTCRtpSenderBase : public RTCRtpSenderInternal, public ObserverInterface {
public:
	...
	void SetOutSsrc(uint32_t ssrc) override { _out_set_ssrc = ssrc; }		
	void SetOutRtxSsrc(uint32_t rtx_ssrc) override { _out_set_rtx_ssrc = rtx_ssrc; }	
	absl::optional<uint32_t> GetOutSsrc() const override { return _out_set_ssrc; }	
	absl::optional<uint32_t> GetOutRtxSsrc() const override { return _out_set_rtx_ssrc; }
...
protected:
	...
	RTCRtpSenderBase(rtc::Thread* worker_thread,
					 const std::string& id,
	...
	absl::optional<uint32_t> _out_set_ssrc;		
	absl::optional<uint32_t> _out_set_rtx_ssrc;	
};

// LocalAudioSinkAdapter receives data callback as a sink to the local
// AudioTrack, and passes the data to the sink of AudioSource.
class RTCLocalAudioSinkAdapter : public AudioTrackSinkInterface,
	public cricket::AudioSource {
public:
	RTCLocalAudioSinkAdapter();
	virtual ~RTCLocalAudioSinkAdapter();
...
class RTCAudioRtpSender : public RTCRtpSenderBase {
public:
	...
	// is called. |set_streams_observer| is not owned by this object. If not
	// null, it must be valid at least until this sender becomes stopped.
	static rtc::scoped_refptr<RTCAudioRtpSender> Create(
		rtc::Thread* worker_thread,
		const std::string& id,
		SetStreamsObserver* set_streams_observer);
	virtual ~RTCAudioRtpSender();
...
protected:
	RTCAudioRtpSender(rtc::Thread* worker_thread,
		const std::string& id,
		SetStreamsObserver* set_streams_observer);
...
private:
	...
	rtc::scoped_refptr<RTCAudioTrackInterface> audio_track() const {
		return rtc::scoped_refptr<RTCAudioTrackInterface>(
			static_cast<RTCAudioTrackInterface*>(track_.get()));
	}

	bool cached_track_enabled_ = false;
};

class RTCVideoRtpSender : public RTCRtpSenderBase {
public:
	...
	// is called. |set_streams_observer| is not owned by this object. If not
	// null, it must be valid at least until this sender becomes stopped.
	static rtc::scoped_refptr<RTCVideoRtpSender> Create(
		rtc::Thread* worker_thread,
		const std::string& id,
		SetStreamsObserver* set_streams_observer);
	virtual ~RTCVideoRtpSender();
...
protected:
	RTCVideoRtpSender(rtc::Thread* worker_thread,
		const std::string& id,
		SetStreamsObserver* set_streams_observer);
