...
constexpr size_t RTCRtcpReceiver::RegisteredSsrcs::kMediaSsrcIndex;
constexpr size_t RTCRtcpReceiver::RegisteredSsrcs::kMaxSsrcs;

RTCRtcpReceiver::RegisteredSsrcs::RegisteredSsrcs(
...
// 	}
	...
	RTC_DCHECK_LE(ssrcs_.size(), RTCRtcpReceiver::RegisteredSsrcs::kMaxSsrcs);
}

bool RTCRtcpReceiver::RegisteredSsrcs::contains(uint32_t ssrc) const {
	RTC_DCHECK_RUN_ON(&packet_sequence_checker_);
	return absl::c_linear_search(ssrcs_, ssrc);
}

uint32_t RTCRtcpReceiver::RegisteredSsrcs::media_ssrc() const {
	RTC_DCHECK_RUN_ON(&packet_sequence_checker_);
	return ssrcs_[kMediaSsrcIndex];
}

void RTCRtcpReceiver::RegisteredSsrcs::set_media_ssrc(uint32_t ssrc) {
	RTC_DCHECK_RUN_ON(&packet_sequence_checker_);
	ssrcs_[kMediaSsrcIndex] = ssrc;
}

struct RTCRtcpReceiver::PacketInformation {
...
RTCRtcpReceiver::RTCRtcpReceiver(const RTCRtpRtcpInterface::Configuration& config,
	ModuleRtpRtcp* owner)
	: clock_(config.clock),
...
RTCRtcpReceiver::~RTCRtcpReceiver() {}

void RTCRtcpReceiver::IncomingPacket(rtc::ArrayView<const uint8_t> packet) {
	if (packet.empty()) {
...
// remove it soon.
int64_t RTCRtcpReceiver::LastReceivedReportBlockMs() const {
	MutexLock lock(&rtcp_receiver_lock_);
	return last_received_rb_.IsFinite() ? last_received_rb_.ms() : 0;
}

void RTCRtcpReceiver::SetRemoteSSRC(uint32_t ssrc) {
	MutexLock lock(&rtcp_receiver_lock_);
	// New SSRC reset old reports.
	remote_sender_.last_arrival_timestamp.Reset();
	last_received_sr_ntp_.Reset();	// del
	remote_ssrc_ = ssrc;
}

void RTCRtcpReceiver::set_local_media_ssrc(uint32_t ssrc) {
	registered_ssrcs_.set_media_ssrc(ssrc);
}

uint32_t RTCRtcpReceiver::local_media_ssrc() const {
	return registered_ssrcs_.media_ssrc();
}

uint32_t RTCRtcpReceiver::RemoteSSRC() const {
	MutexLock lock(&rtcp_receiver_lock_);
	return remote_ssrc_;
}

void RTCRtcpReceiver::RttStats::AddRtt(TimeDelta rtt) {
	last_rtt_ = rtt;
// 	if (rtt < min_rtt_) {
// 		min_rtt_ = rtt;
// 	}
...
absl::optional<TimeDelta> RTCRtcpReceiver::AverageRtt() const {
	MutexLock lock(&rtcp_receiver_lock_);
...
absl::optional<TimeDelta> RTCRtcpReceiver::LastRtt() const {
	MutexLock lock(&rtcp_receiver_lock_);
...
RTCRtcpReceiver::NonSenderRttStats RTCRtcpReceiver::GetNonSenderRTT() const {
	MutexLock lock(&rtcp_receiver_lock_);
...
void RTCRtcpReceiver::SetNonSenderRttMeasurement(bool enabled) {
	MutexLock lock(&rtcp_receiver_lock_);
	xr_rrtr_status_ = enabled;
}

bool RTCRtcpReceiver::GetAndResetXrRrRtt(int64_t* rtt_ms) {
	RTC_DCHECK(rtt_ms);
	MutexLock lock(&rtcp_receiver_lock_);
	if (xr_rr_rtt_ms_ == 0) {
		return false;
...
absl::optional<TimeDelta> RTCRtcpReceiver::OnPeriodicRttUpdate(
...
// int32_t RTCRtcpReceiver::StatisticsReceived(
...
RTCRtcpReceiver::ConsumeReceivedXrReferenceTimeInfo() {
	MutexLock lock(&rtcp_receiver_lock_);
...
std::vector<ReportBlockData> RTCRtcpReceiver::GetLatestReportBlockData() const {
...
bool RTCRtcpReceiver::ParseCompoundPacket(rtc::ArrayView<const uint8_t> packet,
	PacketInformation* packet_information) {
	MutexLock lock(&rtcp_receiver_lock_);
...
void RTCRtcpReceiver::HandleSenderReport(const CommonHeader& rtcp_block,
	PacketInformation* packet_information) {
...
void RTCRtcpReceiver::HandleReceiverReport(const CommonHeader& rtcp_block,
	PacketInformation* packet_information) {
...
void RTCRtcpReceiver::HandleReportBlock(const ReportBlock& report_block,
	PacketInformation* packet_information,
	...
	if (send_time_ntp != 0) {
//		RTC_LOG(LS_INFO) << "rtcp_trace_report RTCRtcpReceiver::HandleReportBlock send_time_ntp " << send_time_ntp;
...
RTCRtcpReceiver::TmmbrInformation* RTCRtcpReceiver::FindOrCreateTmmbrInfo(
	uint32_t remote_ssrc) {
...
void RTCRtcpReceiver::UpdateTmmbrRemoteIsAlive(uint32_t remote_ssrc) {
...
RTCRtcpReceiver::TmmbrInformation* RTCRtcpReceiver::GetTmmbrInformation(
	uint32_t remote_ssrc) {
...
bool RTCRtcpReceiver::RtcpRrTimeout() {
	MutexLock lock(&rtcp_receiver_lock_);
	return RtcpRrTimeoutLocked(clock_->CurrentTime());
}

bool RTCRtcpReceiver::RtcpRrSequenceNumberTimeout() {
	MutexLock lock(&rtcp_receiver_lock_);
	return RtcpRrSequenceNumberTimeoutLocked(clock_->CurrentTime());
}

bool RTCRtcpReceiver::UpdateTmmbrTimers() {
	MutexLock lock(&rtcp_receiver_lock_);
...
std::vector<rtcp::TmmbItem> RTCRtcpReceiver::BoundingSet(bool* tmmbr_owner) {
	MutexLock lock(&rtcp_receiver_lock_);
...
void RTCRtcpReceiver::HandleSdes(const CommonHeader& rtcp_block,
	PacketInformation* packet_information) {
	rtcp::Sdes sdes;
...
void RTCRtcpReceiver::HandleNack(const CommonHeader& rtcp_block,
	PacketInformation* packet_information) {
	rtcp::Nack nack;
...
void RTCRtcpReceiver::HandleApp(const rtcp::CommonHeader& rtcp_block,
	PacketInformation* packet_information) {
	rtcp::App app;
...
void RTCRtcpReceiver::HandleBye(const CommonHeader& rtcp_block) {
	rtcp::Bye bye;
...
void RTCRtcpReceiver::HandleXr(const CommonHeader& rtcp_block,
	PacketInformation* packet_information,
	bool& contains_dlrr,
	uint32_t& ssrc) {
...
void RTCRtcpReceiver::HandleXrReceiveReferenceTime(uint32_t sender_ssrc,
...
void RTCRtcpReceiver::HandleXrDlrrReportBlock(uint32_t sender_ssrc,
...
void RTCRtcpReceiver::HandleXrTargetBitrate(
	uint32_t ssrc,
...
void RTCRtcpReceiver::HandlePli(const CommonHeader& rtcp_block,
	PacketInformation* packet_information) {
	rtcp::Pli pli;
...
void RTCRtcpReceiver::HandleTmmbr(const CommonHeader& rtcp_block,
	PacketInformation* packet_information) {
	rtcp::Tmmbr tmmbr;
...
void RTCRtcpReceiver::HandleTmmbn(const CommonHeader& rtcp_block,
	PacketInformation* packet_information) {
	rtcp::Tmmbn tmmbn;
...
void RTCRtcpReceiver::HandleSrReq(const CommonHeader& rtcp_block,
	PacketInformation* packet_information) {
...
void RTCRtcpReceiver::HandlePsfbApp(const CommonHeader& rtcp_block,
	PacketInformation* packet_information) {
	{
		rtcp::Remb remb;
...
void RTCRtcpReceiver::HandleFir(const CommonHeader& rtcp_block,
	PacketInformation* packet_information) {
	rtcp::Fir fir;
...
void RTCRtcpReceiver::HandleTransportFeedback(
...
void RTCRtcpReceiver::NotifyTmmbrUpdated() {
	// Find bounding set.
...
absl::optional<RTCRtpRtcpInterface::RTCSenderReportStats> RTCRtcpReceiver::GetSenderReportStats() const {
	MutexLock lock(&rtcp_receiver_lock_);
...
void RTCRtcpReceiver::TriggerCallbacksFromRtcpPacket(
...
std::vector<rtcp::TmmbItem> RTCRtcpReceiver::TmmbrReceived() {
	MutexLock lock(&rtcp_receiver_lock_);
...
bool RTCRtcpReceiver::RtcpRrTimeoutLocked(Timestamp now) {
	return ResetTimestampIfExpired(now, last_received_rb_, report_interval_);
}

bool RTCRtcpReceiver::RtcpRrSequenceNumberTimeoutLocked(Timestamp now) {
