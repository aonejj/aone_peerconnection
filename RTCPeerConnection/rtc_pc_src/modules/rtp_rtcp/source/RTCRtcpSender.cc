...
class RTCRtcpSender::PacketSender {
public:
...
RTCRtcpSender::FeedbackState::FeedbackState()
	: packets_sent(0),
	media_bytes_sent(0),
	send_bitrate(0),
// 	last_rr_ntp_secs(0),
...
RTCRtcpSender::FeedbackState::FeedbackState(const FeedbackState&) = default;

RTCRtcpSender::FeedbackState::FeedbackState(FeedbackState&&) = default;

RTCRtcpSender::FeedbackState::~FeedbackState() = default;

std::string RTCRtcpSender::FeedbackState::ToString() const {
	char ss_buf[1024];
	rtc::SimpleStringBuilder ss(ss_buf);
	ss << "feed_trace ";
	ss << "{ packets_sent: " << packets_sent;
	ss << ", media_bytes_sent: " << media_bytes_sent;
	ss << ", send_bitrate: " << send_bitrate;
	ss << ", last_rr: " << last_rr.ToMs();
	ss << ", remote_sr: " << remote_sr;
	ss << ", last_xr_rtis size " << last_xr_rtis.size();
	ss << '}';
	return ss.str();
}

class RTCRtcpSender::RtcpContext {
public:
...
RTCRtcpSender::Configuration RTCRtcpSender::Configuration::FromRtpRtcpConfiguration(
	const RTCRtpRtcpInterface::Configuration& configuration) {
	RTCRtcpSender::Configuration result;
...
RTCRtcpSender::Configuration RTCRtcpSender::AddRtcpSendEvaluationCallback(
...
RTCRtcpSender::RTCRtcpSender(Configuration config)
	: audio_(config.audio),
	...
	builders_[kRtcpSr] = &RTCRtcpSender::BuildSR;
	builders_[kRtcpRr] = &RTCRtcpSender::BuildRR;
	builders_[kRtcpSdes] = &RTCRtcpSender::BuildSDES;
	builders_[kRtcpPli] = &RTCRtcpSender::BuildPLI;
	builders_[kRtcpFir] = &RTCRtcpSender::BuildFIR;
	builders_[kRtcpRemb] = &RTCRtcpSender::BuildREMB;
	builders_[kRtcpBye] = &RTCRtcpSender::BuildBYE;
	builders_[kRtcpLossNotification] = &RTCRtcpSender::BuildLossNotification;
	builders_[kRtcpTmmbr] = &RTCRtcpSender::BuildTMMBR;
	builders_[kRtcpTmmbn] = &RTCRtcpSender::BuildTMMBN;
	builders_[kRtcpNack] = &RTCRtcpSender::BuildNACK;
	builders_[kRtcpAnyExtendedReports] = &RTCRtcpSender::BuildExtendedReports;
}

RTCRtcpSender::~RTCRtcpSender() {}

RtcpMode RTCRtcpSender::Status() const {
	MutexLock lock(&mutex_rtcp_sender_);
	return method_;
}

void RTCRtcpSender::SetRTCPStatus(RtcpMode new_method) {
	MutexLock lock(&mutex_rtcp_sender_);
...
bool RTCRtcpSender::Sending() const {
	MutexLock lock(&mutex_rtcp_sender_);
	return sending_;
}

void RTCRtcpSender::SetSendingStatus(const FeedbackState& feedback_state,
	bool sending) {
...
void RTCRtcpSender::SetNonSenderRttMeasurement(bool enabled) {
	MutexLock lock(&mutex_rtcp_sender_);
	xr_send_receiver_reference_time_enabled_ = enabled;
}

int32_t RTCRtcpSender::SendLossNotification(const FeedbackState& feedback_state,
...
void RTCRtcpSender::SetRemb(int64_t bitrate_bps, std::vector<uint32_t> ssrcs) {
...
void RTCRtcpSender::UnsetRemb() {
	MutexLock lock(&mutex_rtcp_sender_);
	// Stop sending REMB each report until it is reenabled and REMB data set.
	ConsumeFlag(kRtcpRemb, /*forced=*/true);
}

bool RTCRtcpSender::TMMBR() const {
	MutexLock lock(&mutex_rtcp_sender_);
	return IsFlagPresent(RTCPPacketType::kRtcpTmmbr);
}

void RTCRtcpSender::SetMaxRtpPacketSize(size_t max_packet_size) {
	MutexLock lock(&mutex_rtcp_sender_);
	max_packet_size_ = max_packet_size;
}

void RTCRtcpSender::SetTimestampOffset(uint32_t timestamp_offset) {
	MutexLock lock(&mutex_rtcp_sender_);
	timestamp_offset_ = timestamp_offset;
}

void RTCRtcpSender::SetLastRtpTime(uint32_t rtp_timestamp,
...
void RTCRtcpSender::SetRtpClockRate(int8_t payload_type, int rtp_clock_rate_hz) {
	MutexLock lock(&mutex_rtcp_sender_);
	rtp_clock_rates_khz_[payload_type] = rtp_clock_rate_hz / 1000;
}

uint32_t RTCRtcpSender::SSRC() const {
	MutexLock lock(&mutex_rtcp_sender_);
	return ssrc_;
}

void RTCRtcpSender::SetSsrc(uint32_t ssrc) {
	MutexLock lock(&mutex_rtcp_sender_);
	ssrc_ = ssrc;
}

void RTCRtcpSender::SetRemoteSSRC(uint32_t ssrc) {
	MutexLock lock(&mutex_rtcp_sender_);
	remote_ssrc_ = ssrc;
}

int32_t RTCRtcpSender::SetCNAME(absl::string_view c_name) {
...
bool RTCRtcpSender::TimeToSendRTCPReport(bool sendKeyframeBeforeRTP) const {
	/*
...
void RTCRtcpSender::BuildSR(const RtcpContext& ctx, PacketSender& sender) {
...
void RTCRtcpSender::BuildSDES(const RtcpContext& ctx, PacketSender& sender) {
...
void RTCRtcpSender::BuildRR(const RtcpContext& ctx, PacketSender& sender) {
...
void RTCRtcpSender::BuildPLI(const RtcpContext& ctx, PacketSender& sender) {
	rtcp::Pli pli;
...
void RTCRtcpSender::BuildFIR(const RtcpContext& ctx, PacketSender& sender) {
...
void RTCRtcpSender::BuildREMB(const RtcpContext& ctx, PacketSender& sender) {
	rtcp::Remb remb;
...
void RTCRtcpSender::SetTargetBitrate(unsigned int target_bitrate) {
	MutexLock lock(&mutex_rtcp_sender_);
	tmmbr_send_bps_ = target_bitrate;
}

void RTCRtcpSender::BuildTMMBR(const RtcpContext& ctx, PacketSender& sender) {
...
void RTCRtcpSender::BuildTMMBN(const RtcpContext& ctx, PacketSender& sender) {
	rtcp::Tmmbn tmmbn;
...
void RTCRtcpSender::BuildAPP(const RtcpContext& ctx, PacketSender& sender) {
	rtcp::App app;
	app.SetSenderSsrc(ssrc_);
	sender.AppendPacket(app);
}

void RTCRtcpSender::BuildLossNotification(const RtcpContext& ctx,
	PacketSender& sender) {
...
void RTCRtcpSender::BuildNACK(const RtcpContext& ctx, PacketSender& sender) {
	rtcp::Nack nack;
...
void RTCRtcpSender::BuildBYE(const RtcpContext& ctx, PacketSender& sender) {
	rtcp::Bye bye;
...
void RTCRtcpSender::BuildExtendedReports(const RtcpContext& ctx,
	PacketSender& sender) {
...
int32_t RTCRtcpSender::SendRTCP(const FeedbackState& feedback_state,
...
absl::optional<int32_t> RTCRtcpSender::ComputeCompoundRTCPPacket(
...
void RTCRtcpSender::PrepareReport(const FeedbackState& feedback_state) {
...
std::vector<rtcp::ReportBlock> RTCRtcpSender::CreateReportBlocks(
	const FeedbackState& feedback_state) {
...
void RTCRtcpSender::SetCsrcs(const std::vector<uint32_t>& csrcs) {
	RTC_DCHECK_LE(csrcs.size(), kRtpCsrcSize);
	MutexLock lock(&mutex_rtcp_sender_);
	csrcs_ = csrcs;
}

void RTCRtcpSender::SetTmmbn(std::vector<rtcp::TmmbItem> bounding_set) {
	MutexLock lock(&mutex_rtcp_sender_);
	tmmbn_to_send_ = std::move(bounding_set);
	SetFlag(kRtcpTmmbn, true);
}

void RTCRtcpSender::SetFlag(uint32_t type, bool is_volatile) {
...
bool RTCRtcpSender::IsFlagPresent(uint32_t type) const {
	return report_flags_.find(ReportFlag(type, false)) != report_flags_.end();
}

bool RTCRtcpSender::ConsumeFlag(uint32_t type, bool forced) {
...
bool RTCRtcpSender::AllVolatileFlagsConsumed() const {
...
void RTCRtcpSender::SetVideoBitrateAllocation(
...
absl::optional<VideoBitrateAllocation> RTCRtcpSender::CheckAndUpdateLayerStructure(
...
void RTCRtcpSender::SendCombinedRtcpPacket(
...
void RTCRtcpSender::SetNextRtcpSendEvaluationDuration(TimeDelta duration) {
