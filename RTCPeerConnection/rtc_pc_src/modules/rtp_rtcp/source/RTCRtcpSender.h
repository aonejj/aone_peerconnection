...
#ifndef __RTC_RTCP_SENDER_H__
#define __RTC_RTCP_SENDER_H__
...
class RTCRtcpReceiver;

class RTCRtcpSender final {
public:
	...
	struct FeedbackState {
		...
		std::string ToString() const;	// add kimi
		...
		RTCRtcpReceiver* receiver;
	};

	static Configuration AddRtcpSendEvaluationCallback(
		Configuration config,
		std::function<void(TimeDelta)> send_evaluation_callback);

	explicit RTCRtcpSender(Configuration config);

	RTCRtcpSender() = delete;
	RTCRtcpSender(const RTCRtcpSender&) = delete;
	RTCRtcpSender& operator=(const RTCRtcpSender&) = delete;

	virtual ~RTCRtcpSender();
...
private:
	...
	typedef void (RTCRtcpSender::*BuilderFunc)(const RtcpContext&, PacketSender&);
