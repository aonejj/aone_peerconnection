...
//
// author : kimi
// desc   : replace to  /module/pacing/pacing_controller.cc
//
//////////////////////////////////////////////////////////////////////////
...
const TimeDelta RTCPacingController::kMaxExpectedQueueLength = TimeDelta::Millis(2000);
const TimeDelta RTCPacingController::kPausedProcessInterval = kCongestedPacketInterval;
const TimeDelta RTCPacingController::kMinSleepTime = TimeDelta::Millis(1);
const TimeDelta RTCPacingController::kTargetPaddingDuration = TimeDelta::Millis(5);
const TimeDelta RTCPacingController::kMaxPaddingReplayDuration = TimeDelta::Millis(50);
const TimeDelta RTCPacingController::kMaxEarlyProbeProcessing = TimeDelta::Millis(1);

RTCPacingController::RTCPacingController(Clock* clock,
...
RTCPacingController::~RTCPacingController() = default;

void RTCPacingController::CreateProbeClusters(
...
void RTCPacingController::Pause() {
	if (!paused_) {
...
void RTCPacingController::Resume() {
	if (paused_) {
...
bool RTCPacingController::IsPaused() const {
	return paused_;
}

void RTCPacingController::SetCongested(bool congested) {
...
void RTCPacingController::SetCircuitBreakerThreshold(int num_iterations) {
	circuit_breaker_threshold_ = num_iterations;
}

void RTCPacingController::RemovePacketsForSsrc(uint32_t ssrc) {
	packet_queue_.RemovePacketsForSsrc(ssrc);
}

bool RTCPacingController::IsProbing() const {
	return prober_.is_probing();
}

// add kimi... for test
bool RTCPacingController::IsEmptyOrPause() {
	if (paused_ || packet_queue_.Empty()) {
		return true;
	}
	return false;
}
// add kimi... for test
void RTCPacingController::ElapsedDebtUpdate(Timestamp now) {
	UpdateBudgetWithElapsedTime(UpdateTimeAndGetElapsed(now));
}

Timestamp RTCPacingController::CurrentTime() const {
...
void RTCPacingController::SetProbingEnabled(bool enabled) {
	RTC_CHECK(!seen_first_packet_);
	prober_.SetEnabled(enabled);
}

void RTCPacingController::SetPacingRates(DataRate pacing_rate,
...
void RTCPacingController::EnqueuePacket(std::unique_ptr<RtpPacketToSend> packet) {
...
void RTCPacingController::SetAccountForAudioPackets(bool account_for_audio) {
	account_for_audio_ = account_for_audio;
}

void RTCPacingController::SetIncludeOverhead() {
	include_overhead_ = true;
}

void RTCPacingController::SetTransportOverhead(DataSize overhead_per_packet) {
	if (ignore_transport_overhead_)
		return;
	transport_overhead_per_packet_ = overhead_per_packet;
}

void RTCPacingController::SetSendBurstInterval(TimeDelta burst_interval) {
	send_burst_interval_ = burst_interval;
}

TimeDelta RTCPacingController::ExpectedQueueTime() const {
	RTC_DCHECK_GT(adjusted_media_rate_, DataRate::Zero());
	return QueueSizeData() / adjusted_media_rate_;
}

size_t RTCPacingController::QueueSizePackets() const {
	return rtc::checked_cast<size_t>(packet_queue_.SizeInPackets());
}

const std::array<int, kNumMediaTypes>&
RTCPacingController::SizeInPacketsPerRtpPacketMediaType() const {
	return packet_queue_.SizeInPacketsPerRtpPacketMediaType();
}

DataSize RTCPacingController::QueueSizeData() const {
...
DataSize RTCPacingController::CurrentBufferLevel() const {
	return std::max(media_debt_, padding_debt_);
}

absl::optional<Timestamp> RTCPacingController::FirstSentPacketTime() const {
	return first_sent_packet_time_;
}

Timestamp RTCPacingController::OldestPacketEnqueueTime() const {
	return packet_queue_.OldestEnqueueTime();
}

TimeDelta RTCPacingController::UpdateTimeAndGetElapsed(Timestamp now) {
...
bool RTCPacingController::ShouldSendKeepalive(Timestamp now) const {
...
Timestamp RTCPacingController::NextSendTime() const {
	const Timestamp now = CurrentTime();
	...
	if (adjusted_media_rate_ > DataRate::Zero() && !packet_queue_.Empty()) {
		...
		// Nothing to do.
		RTC_LOG(LS_INFO) << "pacing_trace NextSendTime Nothing to do";
...
void RTCPacingController::ProcessPackets() {
...
DataSize RTCPacingController::PaddingToAdd(DataSize recommended_probe_size,
...
std::unique_ptr<RtpPacketToSend> RTCPacingController::GetPendingPacket(
...
void RTCPacingController::OnPacketSent(RtpPacketMediaType packet_type,
...
void RTCPacingController::UpdateBudgetWithElapsedTime(TimeDelta delta) {
	media_debt_ -= std::min(media_debt_, adjusted_media_rate_ * delta);
	padding_debt_ -= std::min(padding_debt_, padding_rate_ * delta);
}

void RTCPacingController::UpdateBudgetWithSentData(DataSize size) {
	media_debt_ += size;
	media_debt_ = std::min(media_debt_, adjusted_media_rate_ * kMaxDebtInTime);
	UpdatePaddingBudgetWithSentData(size);
}

void RTCPacingController::UpdatePaddingBudgetWithSentData(DataSize size) {
	padding_debt_ += size;
	padding_debt_ = std::min(padding_debt_, padding_rate_ * kMaxDebtInTime);
}

void RTCPacingController::SetQueueTimeLimit(TimeDelta limit) {
	queue_time_limit_ = limit;
}

void RTCPacingController::MaybeUpdateMediaRateDueToLongQueue(Timestamp now) {
...
Timestamp RTCPacingController::NextUnpacedSendTime() const {
	if (!pace_audio_) {
