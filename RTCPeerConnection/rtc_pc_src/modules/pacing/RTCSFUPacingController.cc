...
namespace webrtc {
namespace {
	constexpr TimeDelta kCongestedPacketInterval = TimeDelta::Millis(300);		// origin 500
	constexpr TimeDelta kMaxDebtInTime = TimeDelta::Millis(500);
	constexpr TimeDelta kMaxElapsedTime = TimeDelta::Seconds(2);
}

const TimeDelta RTCSFUPacingController::kMaxExpectedQueueLength = TimeDelta::Millis(2000);
const TimeDelta RTCSFUPacingController::kPausedProcessInterval = kCongestedPacketInterval;
const TimeDelta RTCSFUPacingController::kMinSleepTime = TimeDelta::Millis(1);
const TimeDelta RTCSFUPacingController::kTargetPaddingDuration = TimeDelta::Millis(5);
const TimeDelta RTCSFUPacingController::kMaxPaddingReplayDuration = TimeDelta::Millis(50);
const TimeDelta RTCSFUPacingController::kMaxEarlyProbeProcessing = TimeDelta::Millis(1);

RTCSFUPacingController::RTCSFUPacingController(Clock *clock,
...
RTCSFUPacingController::~RTCSFUPacingController() {

}

void RTCSFUPacingController::Pause() {
	if (!paused_) {
...
void RTCSFUPacingController::Resume() {
	if (paused_) {
...
bool RTCSFUPacingController::IsPaused() const {
	return paused_;
}

void RTCSFUPacingController::SetCongested(bool congested) {
...
void RTCSFUPacingController::SetCircuitBreakerThreshold(int num_iterations) {
	circuit_breaker_threshold_ = num_iterations;
}

void RTCSFUPacingController::RemovePacketsForSsrc(uint32_t ssrc) {
	packet_queue_.RemovePacketsForSsrc(ssrc);
}

Timestamp RTCSFUPacingController::CurrentTime() const {
...
void RTCSFUPacingController::SetPacingRates(DataRate pacing_rate,
...
void RTCSFUPacingController::EnqueuePacket(std::unique_ptr<RtpPacketToSend> packet,
										   BitrateProber* prober) {
	RTC_DCHECK(pacing_rate_ > DataRate::Zero())
		<< "SetPacingRate must be called before InsertPacket.";
	RTC_CHECK(packet->packet_type());

	prober->OnIncomingPacket(DataSize::Bytes(packet->payload_size()));
	...
	if (packet_queue_.Empty()) {
		...
		Timestamp next_send_time = NextSendTime(prober);
...
void RTCSFUPacingController::SetIncludeOverhead() {
	include_overhead_ = true;
}

void RTCSFUPacingController::SetTransportOverhead(DataSize overhead_per_packet) {
	if (ignore_transport_overhead_)
		return;
	transport_overhead_per_packet_ = overhead_per_packet;
}

void RTCSFUPacingController::SetSendBurstInterval(TimeDelta burst_interval) {
	send_burst_interval_ = burst_interval;
}

TimeDelta RTCSFUPacingController::ExpectedQueueTime() const {
	RTC_DCHECK_GT(adjusted_media_rate_, DataRate::Zero());
	return QueueSizeData() / adjusted_media_rate_;
}

size_t RTCSFUPacingController::QueueSizePackets() const {
	return rtc::checked_cast<size_t>(packet_queue_.SizeInPackets());
}

DataSize RTCSFUPacingController::QueueSizeData() const {
...
DataSize RTCSFUPacingController::CurrentBufferLevel() const {
	return std::max(media_debt_, padding_debt_);
}

absl::optional<Timestamp> RTCSFUPacingController::FirstSentPacketTime() const {
	return first_sent_packet_time_;
}

Timestamp RTCSFUPacingController::OldestPacketEnqueueTime() const {
	return packet_queue_.OldestEnqueueTime();
}

TimeDelta RTCSFUPacingController::UpdateTimeAndGetElapsed(Timestamp now) {
...
bool RTCSFUPacingController::ShouldSendKeepalive(Timestamp now) const {
...
Timestamp RTCSFUPacingController::NextSendTime(BitrateProber* prober) const {
	const Timestamp now = CurrentTime();
	...
	if (prober->is_probing() && !probing_send_failure_) {
		Timestamp probe_time = prober->NextProbeTime(now);
...
void RTCSFUPacingController::ProcessPackets(BitrateProber* prober) {
		...
		if (seen_first_packet_) {
			...
			for (auto& packet : keepalive_packets) {
				...
				for (auto& packet : packet_sender_->FetchFec()) {
					EnqueuePacket(std::move(packet), prober);
				}
			}
		}
	...
	TimeDelta early_execute_margin =
		prober->is_probing() ? kMaxEarlyProbeProcessing : TimeDelta::Zero();

	target_send_time = NextSendTime(prober);
	...
	if (is_probing) {
		...
		if (!probing_send_failure_) {
			prober->ProbeSent(CurrentTime(), data_sent);
		}
	}
...
DataSize RTCSFUPacingController::PaddingToAdd(DataSize recommended_probe_size,
...
std::unique_ptr<RtpPacketToSend> RTCSFUPacingController::GetPendingPacket(
...
void RTCSFUPacingController::OnPacketSent(RtpPacketMediaType packet_type,
...
void RTCSFUPacingController::UpdateBudgetWithElapsedTime(TimeDelta delta) {
	media_debt_ -= std::min(media_debt_, adjusted_media_rate_ * delta);
	padding_debt_ -= std::min(padding_debt_, padding_rate_ * delta);
}

void RTCSFUPacingController::UpdateBudgetWithSentData(DataSize size) {
	media_debt_ += size;
	media_debt_ = std::min(media_debt_, adjusted_media_rate_ * kMaxDebtInTime);
	UpdatePaddingBudgetWithSentData(size);
}

void RTCSFUPacingController::UpdatePaddingBudgetWithSentData(DataSize size) {
	padding_debt_ += size;
	padding_debt_ = std::min(padding_debt_, padding_rate_ * kMaxDebtInTime);
}

void RTCSFUPacingController::SetQueueTimeLimit(TimeDelta limit) {
	queue_time_limit_ = limit;
}

void RTCSFUPacingController::MaybeUpdateMediaRateDueToLongQueue(Timestamp now) {
...
Timestamp RTCSFUPacingController::NextUnpacedSendTime() const {
	if (!pace_audio_) {
