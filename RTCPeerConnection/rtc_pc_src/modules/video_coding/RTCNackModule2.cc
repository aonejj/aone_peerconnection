...
namespace {
	...
	int64_t GetSendNackDelay() {
		...
		return kDefaultSendNackDelayMs;	// source
	}
}  // namespace

constexpr TimeDelta RTCNackModule2::kUpdateInterval;

RTCNackModule2::NackInfo::NackInfo()
	: seq_num(0), send_at_seq_num(0), sent_at_time(-1), retries(0) {}

RTCNackModule2::NackInfo::NackInfo(uint16_t seq_num,
...
RTCNackModule2::BackoffSettings::BackoffSettings(TimeDelta min_retry,
	TimeDelta max_rtt,
	double base)
	: min_retry_interval(min_retry), max_rtt(max_rtt), base(base) {}

absl::optional<RTCNackModule2::BackoffSettings>
RTCNackModule2::BackoffSettings::ParseFromFieldTrials() {
	...
	if (enabled) {
		return RTCNackModule2::BackoffSettings(min_retry.Get(), max_rtt.Get(),
			base.Get());
	}
	return absl::nullopt;
}

RTCNackModule2::RTCNackModule2(TaskQueueBase* current_queue,
	Clock* clock,
...
RTCNackModule2::~RTCNackModule2() {
	RTC_DCHECK_RUN_ON(worker_thread_);
	repeating_task_.Stop();
}

int RTCNackModule2::OnReceivedPacket(uint16_t seq_num, bool is_keyframe) {
	return OnReceivedPacket(seq_num, is_keyframe, false);
}

int RTCNackModule2::OnReceivedPacket(uint16_t seq_num,
	bool is_keyframe,
	bool is_recovered) {
	MutexLock lock(&nack_modules_mutex_);
	...
	if (AheadOf(newest_seq_num_, seq_num)) {
		...
		if (!is_retransmitted)
			UpdateReorderingStatistics_l(seq_num);
	...
	AddPacketsToNack_l(newest_seq_num_ + 1, seq_num);
	newest_seq_num_ = seq_num;

	// Are there any nacks that are waiting for this seq_num.
	std::vector<uint16_t> nack_batch = GetNackBatch_l(kSeqNumOnly);
	if (!nack_batch.empty()) {
...
void RTCNackModule2::ClearUpTo(uint16_t seq_num) {
	MutexLock lock(&nack_modules_mutex_);
...
void RTCNackModule2::UpdateRtt(int64_t rtt_ms) {
	MutexLock lock(&nack_modules_mutex_);
	rtt_ms_ = rtt_ms;
}

bool RTCNackModule2::RemovePacketsUntilKeyFrame_l() {
...
void RTCNackModule2::AddPacketsToNack_l(uint16_t seq_num_start,
	...
	if (nack_list_.size() + num_new_nacks > kMaxNackPackets) {
		while (RemovePacketsUntilKeyFrame_l() &&
			nack_list_.size() + num_new_nacks > kMaxNackPackets) {
		}

		if (nack_list_.size() + num_new_nacks > kMaxNackPackets) {
			nack_list_.clear();
			RTC_LOG(LS_WARNING) << "keyframe_req req_key_trace NACK list full, clearing NACK"	
	...
	for (uint16_t seq_num = seq_num_start; seq_num != seq_num_end; ++seq_num) {
		...
		if (recovered_list_.find(seq_num) != recovered_list_.end())
			continue;
		NackInfo nack_info(seq_num, seq_num + WaitNumberOfPackets_l(0.5),
...
std::vector<uint16_t> RTCNackModule2::GetNackBatch(NackFilterOptions options) {
	MutexLock lock(&nack_modules_mutex_);
	return GetNackBatch_l(options);
}

std::vector<uint16_t> RTCNackModule2::GetNackBatch_l(NackFilterOptions options) {
	// Called on worker_thread_.
...
void RTCNackModule2::UpdateReorderingStatistics_l(uint16_t seq_num) {
...
int RTCNackModule2::WaitNumberOfPackets_l(float probability) const {
	// Called on worker_thread_;
